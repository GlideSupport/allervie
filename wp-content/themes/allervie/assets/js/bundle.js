	...
	let b, c, d, e, f, g,
		...
		B = function() {
			const a = document.createElement( 'p' ).style,
				b = [ 'ms', 'O', 'Moz', 'Webkit' ]; if ( void 0 !== a.transition ) {
				return ! 0;
			} for ( ;b.length; ) {
		...
		let e; if ( c.isObj === ! 1 ) {
			b.items = c.items.toArray(), b.index = 0; let g,
			...
			b.updateSize();
		} ), b.st.closeOnContentClick || ( f += ' mfp-auto-cursor' ), f && b.wrap.addClass( f ); const k = b.wH = v.height(),
		...
		M = function( c ) {
			if ( c.data && void 0 !== c.data.title ) {
				return c.data.title;
			} const d = b.st.image.titleSrc; if ( d ) {
				if ( a.isFunction( d ) ) {
					return d.call( b, c );
				} if ( c.el ) {
					return c.el.attr( d ) || '';
		...
		let d; d = c ? b.currItem.img : b.st.zoom.opener( b.currItem.el || b.currItem ); const e = d.offset(),
			f = parseInt( d.css( 'padding-top' ), 10 ),
			g = parseInt( d.css( 'padding-bottom' ), 10 ); e.top -= a( window ).scrollTop() - f; const h = { width: d.width(), height: ( u ? d.innerHeight() : d[ 0 ].offsetHeight ) - g - f }; return O() ? h[ '-moz-transform' ] = h.transform = 'translate(' + e.left + 'px,' + e.top + 'px)' : ( h.left = e.left, h.top = e.top ), h;
	} } } ); const P = 'iframe',
		Q = '//about:blank',
		R = function( a ) {
			if ( b.currTemplate[ P ] ) {
