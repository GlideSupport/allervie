...
if ( function_exists( 'acf_add_options_page' ) ) {
	$option_page = acf_add_options_page(
		array(
			'page_title' => __( 'Theme Options', 'alrv_td' ),
			'menu_title' => __( 'Theme Options', 'alrv_td' ),
...
function wysiwyg_render_field_settings( $field ) {
	acf_render_field_setting(
		$field,
		array(
			'label'        => __( 'Height of Editor', 'alrv_td' ),
			'instructions' => __( 'Height of Editor after Init', 'alrv_td' ),
			'name'         => 'wysiwyg_height',
			'type'         => 'number',
		)
	);
...
<style type="text/css">
	...
	min-height:
	<?php
	echo $custom_acf_wysiwyg_height;
	?>
	px;
}
</style>
...
jQuery(window).load(function() {
	jQuery('<?php echo $field_class; ?>').each(function() {
		jQuery('#' + jQuery(this).find('iframe').attr('id')).height(
			<?php echo $custom_acf_wysiwyg_height; ?>);
	});
});
</script>
	<?php
