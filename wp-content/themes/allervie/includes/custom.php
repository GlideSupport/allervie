...
function custom_modify_local_business_schema_output($schema) {
	...
	if( have_rows('alrv_dynamic_schema_sameas','options') ){
	 	$link_url = [];
	    while ( have_rows('alrv_dynamic_schema_sameas','options') ) { the_row();
...
function location_assign_closure_blk($post_id, $post, $update) {
    ...
    if($cl_option == 'auto'){ 
		...
		if(!empty($loction) && is_array($loction)){
			update_post_meta($post_id, 'alrv_cpt_cl_slct_location', $loction);
		}
		...
		jQuery(function($) {
			...
			function addDayMessage($wrapper) {
				...
				function showDay() {
					...
					$wrapper.append(`
						<div class="acf-date-day-message"
							style="margin-top:6px;color:#333;">Day 
							${dayName}
						</div>
					`);
				}
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            setTimeout(function () {
                ...
                if (fieldWrapper) {
					...
					if (metaValue == "1") {
						messageText = "You have selected the location as “Auto.”";
					} else if (metaValue == "0") {
						messageText = "You have selected the location as “Manual.”";
					} else {
...
add_filter( 'manage_closure_posts_columns', 'set_custom_edit_closure_columns' );
function set_custom_edit_closure_columns($columns) {
	...
	$columns['alrv_cpt_cl_auto_manual'] = "Status Type";
	$columns['date'] = "Date";
    return $columns;
}
