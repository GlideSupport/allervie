...
$alrv_posttitle = glide_page_title( 'alrv_posttitle' );
...
<div class="container-980">
	<div class="wrapper">
		...
		<div class="post-meta d-flex justify-content-between align-items-center">
			<!-- /.post-tags -->
			<?php if ( $alrv_post_categories ) { ?>
				<div class="post-cat">
					<?php foreach ( $alrv_post_categories as $category ) { ?>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class( 'post-ctn' ); ?>>
			...
			$alrv_rp_selection_criteria = isset( $fields['alrv_rp_selection_criteria'] ) ? $fields['alrv_rp_selection_criteria'] : null;
			if ( $alrv_rp_selection_criteria == 'random' ) {
				...
				global $post;
				$alrv_selected_posts = array();
				$alrv_selected_posts = isset( $fields['alrv_rp_selected_posts'] ) ? $fields['alrv_rp_selected_posts'] : null;
				if ( $alrv_selected_posts ) {

					?>
				 <div class="related-posts ">
				<h3><?php _e( 'Related Posts', 'alrv_td' ); ?></h3>
							  <?php
								foreach ( $alrv_selected_posts as $alrv_post ) {
									$post = $alrv_post;
									setup_postdata( $post );
									$pID          = $post->ID;
