...
if ($cl_qry->have_posts()) {
	...
	while ($cl_qry->have_posts()) {
		...
		if ($cl_loc_op != 1 && !empty($cl_slct_location)) {
				...
				<div class="banner-text center-align">
					...
					<div class="provider-filter closure variation-filter d-flex flex-wrap has-filter-btn">
						...
						<div class="filter-list fiter-btn">
							<button type="button" id="clear-all" class="button cl-clear-btn"
								style="display:none"><?php _e('Clear All', 'alrv_td'); ?></button>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="wrapper">
							...
							<div class="faqs-section faq-variation">
								<div class="faqs-area">
									...
									foreach ($location_dt as $loc_nm => $loc_dt_item) {
										...
										?>
										<div class="faq">
											<h3 class="large-text faq-title d-flex justify-content-between">
												<?php echo $loc_nm; ?><span
													class="faq-numbers"><?php echo $total_locations; ?></span><span
			...
			if ((!empty($dateListPaged) && is_array($dateListPaged)) && ($total_pages > 1)) { ?>
				<div class="center-align load-more-btn">
					<a href="" title="See All" target="" data-page="1" data-max="<?php echo $total_pages; ?>" id="loadMore"
						class="loadmore-btn"><?php _e('Show More', 'alrv_td'); ?></a>
				</div>
				<?php
			}
			?>
